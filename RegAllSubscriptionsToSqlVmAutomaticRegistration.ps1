<#
    .SYNOPSIS
    For all subscriptions accessible to the logged in user, enabled SQL IaaS extension at the subscription level.
    ...
    The cmdlet generates a list of available subscriptions and invokes the bulk registration script to register all Azure VMs registers the VMs running SQL Server on Windows with SQL VM Resource provider

    Registration is enabled in "Light Mode"
    ...
    .EXAMPLE  
        To register ALL subscriptions, simply run this powershell script.


        PS C:\MyScripts\RegAllSubscriptions.ps1
#>
